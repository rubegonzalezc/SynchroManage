// @deprecated — Los correos transaccionales se envían desde Next.js (src/lib/email/send.ts).
// Esta función se mantiene solo como referencia legacy; no la uses en código nuevo.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body.type || !isValidNotificationType(body.type)) {
      return new Response(
        JSON.stringify({
          error: `Invalid notification type: "${body.type}". Must be one of: ${VALID_TYPES.join(', ')}`,
        }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      )
    }

    if (!body.subject || !body.data) {
      return new Response(
        JSON.stringify({ error: 'Missing "subject" or "data" field' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      )
    }
    ...
    const resendResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'SynchroManage <no-reply@synchrodev.cl>',
        to: [body.to],
